{ lib, ... }: {
  imports = [ ./modules/boot/systemd.nix ./modules/systems/server.nix ];

  boot.loader.generic-extlinux-compatible.enable = lib.mkOverride 100 false;  # this is set in the nixos-hardware input

  users.users.user.openssh.authorizedKeys.keys = [
    "ssh-rsa  user@eleum"
  ];

  system.stateVersion = "23.11";
  time.timeZone = "Europe/Paris";
  nixpkgs.config.allowUnfree = true;

  hardware.enableAllFirmware = true;
  networking.hostName = "majula";

  services = {
    fstrim.enable = true;
    fwupd.enable = true;
  };
}

